...
unsigned char
    tm_thread_stack_area[TM_THREADX_MAX_THREADS * TM_THREADX_THREAD_STACK_SIZE];
unsigned char
    tm_queue_memory_area[TM_THREADX_MAX_QUEUES * TM_THREADX_QUEUE_SIZE];
unsigned char tm_pool_memory_area[TM_THREADX_MAX_MEMORY_POOLS *
                                  TM_THREADX_MEMORY_POOL_SIZE];
...
int tm_queue_create(int queue_id) {
  ...
  status = tx_queue_create(
      &tm_queue_array[queue_id], "Thread-Metric test", TX_4_ULONG,
...
int tm_mutex_create(int mutex_id) {
  ...
  status = tx_mutex_create(&tm_mutex_array[mutex_id], "Thread-Metric Mutex",
                           TX_INHERIT);
...
int tm_memory_pool_create(int pool_id) {
  ...
  status = tx_block_pool_create(
      &tm_block_pool_array[pool_id], "Thread-Metric test", 128,
      &tm_pool_memory_area[pool_id * TM_THREADX_MEMORY_POOL_SIZE],
      TM_THREADX_MEMORY_POOL_SIZE);
...
int tm_memory_pool_allocate(int pool_id, unsigned char **memory_ptr) {
  ...
  status = tx_block_allocate(&tm_block_pool_array[pool_id], (void **)memory_ptr,
                             TX_NO_WAIT);
